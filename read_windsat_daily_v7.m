...
% reads version-7 files released November 2011
%
...
data=reshape(fread(fid,xdim*ydim*numvar*tdim,'uint8'),xdim,ydim,numvar,tdim);
fclose(fid);
%disp(data_file);

map=-999.*ones(xdim,ydim,numvar,tdim);
for iasc=1:tdim
    for ivar=1:numvar
       dat1=-999.*ones(xdim,ydim);
       dat=data(:,:,ivar,iasc);
       ia=find(dat<=250);
       dat1(ia)=dat(ia).*xscale(ivar)+offset(ivar);
       map(:,:,ivar,iasc)=dat1;
       clear dat1 ia dat
    end % ivar loop
end % iasc loop
